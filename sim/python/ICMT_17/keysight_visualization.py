...
T_int = .001 			# entire signal length in s
...
a1_fc.tofile("data_a1_seq.txt",format='%.5f',sep=',')
#np.savetxt("data_a1_seq.txt",a1_fc,fmt='%.5f',delimiter=',')
##################### Plots ###########################
...
#plt.grid(True)
#plt.legend(loc='upper left', bbox_to_anchor=(1.12,1.35))
#s = "Pulse-shaped baseband signal, $\\tau={0:6.2}$".format(tau)
#plt.title(s)
#plt.ylabel('$s_{m}(t)$')
#plt.xlabel('Time')
