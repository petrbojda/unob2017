...
plt.plot(t, a3,'-g', label='$x_{rcos}(t), \\beta = 0.0$')
plt.plot(t,  c,'-k', label='$x_{rect}(t)$')
plt.grid(True)
plt.legend(loc='upper right')
plt.title('Pulse-shaped baseband signal $\\tau = 1.0$')
plt.ylabel('Voltage')
plt.xlabel('Time')
...
#ax2 = plt.subplot(312)
#plt.plot(tc, A1_c,'-r', label="$X_{sinc}(f)$")
#plt.plot(tc, A2_c,'-b', label='$X_{rcos}(f)$')
#plt.plot(tc, A3_c,'-g', label='$X_{rcos}(f)$')
#plt.grid(True)

# Frequency domain - spectrum of square pulse
#ax3 = plt.subplot(313,sharex=ax2)
#plt.plot(tc, C1_c,'-r', label="$X_{sinc}(f)$")
#plt.plot(tc, C2_c,'-b', label='$X_{rcos}(f)$')
#plt.plot(tc, C3_c,'-g', label='$X_{rcos}(f)$')
